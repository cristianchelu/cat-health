...
export default function DeviceDetail() {
  ...
  const handleUpdateEvent = async (
    ...
    try {
      await updateEventMutation.mutateAsync({
        eventId,
        data: { data, human_verified, pet_id: pet_id ?? null },
      });
    } catch (error) {
  ...
  const handleUpdateLitterboxEvent = async (
    eventId: number,
    data: {
      type: 'litterbox_use';
  ...
  const handleUpdateMaintenanceEvent = async (
    eventId: number,
    data: {
  ...
  const {
    ...
    error: eventsError,
  } = useDeviceEvents(deviceId, currentDate, isValidId);

  const { data: pets, isLoading: petsLoading } = usePets();

  if (!isValidId) {
    return (
      <div className="device-detail device-detail--error">
        Invalid device ID.
      </div>
    );
  }
  ...
  if (deviceLoading || eventsLoading || petsLoading) {
    return (
      <div className="device-detail device-detail--loading">Loading...</div>
    );
  }
  ...
  if (!device) {
    return (
      <div className="device-detail device-detail--empty">
        Device not found.
      </div>
    );
  }
            ...
            {events
              ...
              .map((event) => {
                ...
                if (
                  ...
                  typeof event.data === 'object' &&
                  'type' in event.data &&
                  event.data.type === 'litterbox_maintenance'
                ) {
                ...
                if (
                  ...
                  typeof event.data === 'object' &&
                  'type' in event.data &&
                  event.data.type === 'weight_measurement'
                ) {
