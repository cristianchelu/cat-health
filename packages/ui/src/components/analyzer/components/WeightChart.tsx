...
const calculateRollingVariance = (
  weights: number[],
  ...
  for (let i = 0; i < weights.length; i++) {
    ...
    const variance = Math.sqrt(
      window.reduce((sum, w) => sum + Math.pow(w - mean, 2), 0) / window.length,
    );
 ...
 */
function createStateAnnotations(
  statePeriods: Array<{ state: string; start: number; end: number }>,
) {
...
const WeightChart = React.forwardRef<HTMLCanvasElement, WeightChartProps>(
  ({ selectedEvent, analysisData, className, catWeights }, ref) => {
    const chartInstanceRef = React.useRef<InstanceType<typeof ChartJS> | null>(
      null,
    );

    React.useEffect(() => {
      if (!analysisData || !selectedEvent || !ref || typeof ref === 'function')
        return;
      ...
      const weights = data.measurements.map(
        (m: { weight: number }) => m.weight,
      );

      // Calculate rolling variance
      const [rollingVariance /* rollingMeans */] =
        calculateRollingVariance(weights);
      const [, /* rollingVariance */ rollingMeans] = calculateRollingVariance(
        weights,
        10,
      );
