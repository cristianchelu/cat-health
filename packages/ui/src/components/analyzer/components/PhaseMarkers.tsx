      ...
      >
        ...
        <div className="phase-markers-grid">
          {periods.map((period) => (
            <span
              key={period.start}
              className={`phase-marker state-${period.state}`}
            >
              {period.state.charAt(0).toUpperCase() + period.state.slice(1)}:{' '}
              {((period.end - period.start) / 10).toFixed(1)}s (
              {period.variance !== undefined
                ? `Var: ${period.variance.toFixed(2)}`
                : 'Var: N/A'}
              )
            </span>
          ))}
        ...
        <div className="metrics-row">
          ...
          <span className="metric">
            Covering Activity:{' '}
            <strong>{features.coveringFluctuations} peaks</strong>
          </span>
        </div>
      </div>
    );
