...
const ScatterAnalysis = React.forwardRef<HTMLDivElement, ScatterAnalysisProps>(
  ({ scatterData, onEventSelect, chartRef, chartInstance }, ref) => {
    ...
    const calculateAxisBounds = () => {
      ...
      const allXValues = scatterData.map((d) =>
        getFeatureValue(d.features, scatterXAxis),
      );
      const allYValues = scatterData.map((d) =>
        getFeatureValue(d.features, scatterYAxis),
      );
    ...
    const filteredScatterData = scatterData.filter((d) => {
      ...
      const xInRange =
        (xMin === undefined || xValue >= xMin) &&
        (xMax === undefined || xValue <= xMax);
      const yInRange =
        (yMin === undefined || yValue >= yMin) &&
        (yMax === undefined || yValue <= yMax);
    ...
    useEffect(() => {
      ...
      const createChart = () => {
        ...
        const datasets = ['urination', 'both', 'defecation', 'unknown']
          .map((type) => {
            const typeData = filteredScatterData.filter(
              (d) => d.eliminationType === type,
            );
        ...
        const xDimension = featureDimensions.find(
          (d) => d.key === scatterXAxis,
        );
        const yDimension = featureDimensions.find(
          (d) => d.key === scatterYAxis,
        );
          ...
          options: {
              ...
              tooltip: {
                callbacks: {
                  label: (context) => {
                    ...
                    const eventId = (context.raw as { eventId: number })
                      .eventId;
                    const event = scatterData.find(
                      (d) => d.event.id === eventId,
                    );
            ...
            onClick: (_, elements) => {
              if (elements.length > 0) {
                ...
                const eventId = (dataset.data[index] as { eventId: number })
                  .eventId;
                const eventToSelect = filteredScatterData.find(
                  (d) => d.event.id === eventId,
                )?.event;
                if (eventToSelect) {
      ...
      <Card className="scatter-analysis" ref={ref}>
        ...
        <div className="scatter-summary">
          <div className="summary-stats">
            <span className="stat">
              Filtered Events: <strong>{filteredScatterData.length}</strong>
            </span>
            <span className="stat">
              Total Events: <strong>{scatterData.length}</strong>
            </span>
            <span className="stat">
              Urination:{' '}
              <strong>
                {
                  filteredScatterData.filter(
                    (d) => d.eliminationType === 'urination',
                  ).length
                }
              </strong>
            </span>
            ...
            <span className="stat">
              ...
              <strong>
                {
                  filteredScatterData.filter(
                    (d) => d.eliminationType === 'defecation',
                  ).length
                }
              </strong>
            </span>
          ...
          <p className="chart-hint">
            💡 Click on any point to analyze that specific event
          </p>
          {filteredScatterData.length < scatterData.length && (
            <p className="filter-notice">
              ⚠️ {scatterData.length - filteredScatterData.length} events
              filtered out
            </p>
          )}
        </div>
