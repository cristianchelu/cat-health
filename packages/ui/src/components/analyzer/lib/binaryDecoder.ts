...
export const decodeRawData = (rawDataArray: number[]): DecodedData => {
  ...
  context.litterRemaining =
    litterRemaining === 65535 ? undefined : litterRemaining;
  offset += 2;
  ...
  context.daysSinceLitterReplaced =
    daysSinceLitterReplaced === 255 ? undefined : daysSinceLitterReplaced;
  offset += 1;

  const hoursSinceLastScoop = dataView.getUint8(offset);
  context.hoursSinceLastScoop =
    hoursSinceLastScoop === 255 ? undefined : hoursSinceLastScoop;
  offset += 1;
