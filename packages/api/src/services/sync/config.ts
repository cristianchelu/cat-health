...
assert(
  process.env.MIGRATION_BATCH_DAYS,
  'MIGRATION_BATCH_DAYS is required in .env',
);
assert(process.env.CAT_WEIGHTS, 'CAT_WEIGHTS is required in .env');
assert(
  process.env.PET_SENSOR_MAPPINGS,
  'PET_SENSOR_MAPPINGS is required in .env',
);
assert(process.env.RECORDING_PATH, 'RECORDING_PATH is required in .env');

// Validate thresholds
const MAINTENANCE_THRESHOLD = parseInt(
  process.env.MAINTENANCE_THRESHOLD || '-20',
);
const NO_ELIMINATION_THRESHOLD = parseInt(
  process.env.NO_ELIMINATION_THRESHOLD || '10',
);

assert(
  MAINTENANCE_THRESHOLD < 0,
  'MAINTENANCE_THRESHOLD should be a negative number.',
);
assert(
  NO_ELIMINATION_THRESHOLD >= 0,
  'NO_ELIMINATION_THRESHOLD should be a non-negative number.',
);
...
export const appConfig = {
  ...
  migration: {
    startDate: new Date(
      process.env.MIGRATION_START_DATE || Date.now() - 48 * 60 * 60 * 1000,
    ),
  ...
  litterbox: {
    ...
    catWeights: process.env.CAT_WEIGHTS.split(',').reduce(
      (acc, pair) => {
    ...
    camera: {
      ...
      timezoneOffsetHours: parseInt(
        process.env.CAMERA_TIMEZONE_OFFSET_HOURS || '3',
      ),
  ...
  weight: {
    petSensorMappings: process.env.PET_SENSOR_MAPPINGS.split(',').reduce(
      (acc, pair) => {
