...
export class WeightMeasurementMigrator implements EventMigrator {
  ...
  async migrate(startDate: Date, endDate: Date): Promise<void> {
    console.log(`\n=== ${this.name} Migration ===`);
    console.log(
      `Processing weight measurements from ${startDate.toISOString()} to ${endDate.toISOString()}`,
    );
    ...
    try {
      ...
      const existingEvents = await this.getExistingWeightEvents(
        startDate,
        endDate,
      );
      console.log(
        `Found ${existingEvents.size} existing weight measurements in time range`,
      );

      const newEvents: NewEvent[] = [];

      // Process each pet sensor mapping
      for (const [sensorName, petId] of Object.entries(
        appConfig.weight.petSensorMappings,
      )) {
        console.log(`\nProcessing ${sensorName} for pet ${petId}...`);

        const measurements = await this.queryWeightMeasurements(
          sensorName,
          startDate,
          endDate,
        );
        console.log(
          `Found ${measurements.length} measurements for ${sensorName}`,
        );
        ...
        for (const measurement of measurements) {
          ...
          if (measurement.weight <= 0) {
            console.warn(
              `Skipping invalid weight measurement at ${measurement.timestamp.toISOString()} with weight ${measurement.weight}`,
            );
            stats.errors++;
            continue;
          }
      ...
      if (newEvents.length > 0) {
        console.log(
          `Inserting ${newEvents.length} new weight measurement events...`,
        );
  ...
  private async getExistingWeightEvents(
    startDate: Date,
    endDate: Date,
  ): Promise<Set<string>> {
