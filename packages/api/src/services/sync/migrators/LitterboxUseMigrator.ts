...
export class LitterboxUseMigrator implements EventMigrator {
  ...
  async migrate(startDate: Date, endDate: Date): Promise<void> {
    console.log(`\n=== ${this.name} Migration ===`);
    console.log(`Processing litterbox events from ${startDate.toISOString()} to ${endDate.toISOString()}`);

    const stats: MigrationStats = { processed: 0, skipped: 0, inserted: 0, errors: 0 };

    try {
      // Check for existing litterbox events
      const existingEvents = await this.getExistingLitterboxEvents(startDate, endDate);
      console.log(`Found ${existingEvents.size} existing litterbox events in time range`);
  ...
  private async getExistingLitterboxEvents(startDate: Date, endDate: Date): Promise<Set<string>> {
        ...
        complete: () => {
          ...
          for (const event of activityEvents) {
            if (event.value === 1 && currentStart === null) {
              ...
              if (duration > 10000) { // At least 10 seconds
                sessions.push({
  ...
  private async processEventSession(session: EventSession): Promise<NewEvent | null> {
    try {
      ...
      const sampleCount = 5;
      const finalRaw = rawMeasurements.slice(-sampleCount).reduce((sum, m) => sum + m.weight, 0) / sampleCount;
      const finalTared = taredMeasurements.slice(-sampleCount).reduce((sum, m) => sum + m.weight, 0) / sampleCount;
      ...
      console.log(`Tare offset: ${tareOffset}g, final tared weight: ${finalTared}g`);
      ...
      const rawData = this.encodeRawData(session.startTime, measurements, contextData || undefined);
      ...
      console.error(`Error processing session at ${session.startTime.toISOString()}:`, error);
      return null;
    }
  }

  private async queryRawWeightData(startTime: Date, endTime: Date): Promise<RawMeasurement[]> {
    const queryApi = this.options.influx.getQueryApi(appConfig.influx.org);
  ...
  private async queryTaredWeightData(startTime: Date, endTime: Date): Promise<RawMeasurement[]> {
    const queryApi = this.options.influx.getQueryApi(appConfig.influx.org);
  ...
  private async queryContextData(timestamp: Date): Promise<ContextData | null> {
    ...
    return new Promise((resolve) => {
      ...
      function finishProcessing() {
        ...
        const daysSinceLitterReplaced = Math.max(0, Math.round(30 - deepCleanTimer));
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (session) {
        if (eventData.type === "litterbox_use") {
          await this.options.mediaService.downloadVideo(session.startTime, session.endTime, 'use');
        } else if (eventData.type === "litterbox_maintenance") {
          await this.options.mediaService.downloadVideo(session.startTime, session.endTime, 'maintenance');
        }
      } else {
        ...
        if (eventData.type === "litterbox_use") {
          ...
          await this.options.mediaService.downloadVideo(event.timestamp, endTime, 'use');
        } else if (eventData.type === "litterbox_maintenance") {
          // For maintenance events, use a fixed 3-minute duration
          const endTime = new Date(event.timestamp.getTime() + 3 * 60 * 1000);
          await this.options.mediaService.downloadVideo(event.timestamp, endTime, 'maintenance');
        }
      }
