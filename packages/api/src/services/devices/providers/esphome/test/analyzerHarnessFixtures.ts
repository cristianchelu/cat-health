/**
 * Fixture loaders extracted so one-off scripts can pull them in without
 * importing `analyzerHarness.test.ts` (which would execute tests on import).
 *
 * Consumers: `analyzerHarness.test.ts`, `postElimEnergyStudy.ts`.
 */
...
export function parseEliminationType(s: string): LitterboxUseEliminationType {
