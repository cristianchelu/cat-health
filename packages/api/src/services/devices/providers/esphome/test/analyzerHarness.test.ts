...
import { loadBouts, loadStream, loadVisits } from './analyzerHarnessData.ts';
export {
  loadBouts,
  loadStream,
  loadVisits,
  ...
  () => {
    it('replay visits, metrics, metrics_latest.json, optional baseline gate', async () => {
      ...
      for (const v of visits) {
        ...
        // Production parity: the analyzer runs at each visit's measured rate
        // (see deriveLitterboxSampleRateHz), not the legacy 10Hz assumption.
        const analyzer = new StateAnalyzer(v.knownGrams, v.sample_rate_hz);
